...
本节只讨论这两个维度。**低代码平台（如 Dify、LangFlow、Flowise）**已经在 8.1 节的“形态 F”中讨论，这里不再和 API / runtime 混成同一张表。
...
**优化策略**（包括提示词缓存、上下文压缩、计划缓存等）详见第 9.4 节。本节重点在于**如何测量和解释 Token 膨胀系数**，而不是在缺乏统一实验条件时轻率比较具体框架的绝对高低。
