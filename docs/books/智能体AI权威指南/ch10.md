...
这里说的 **Agent Loop** 是执行器内部的微观闭环，回答的是“这一轮下一步做什么”。它和后文 10.4 节讨论的任务级 workflow 不是两套互斥概念，而是嵌套关系：
...
因此，Gather-Action-Verify 不是对 Think-Act-Observe 的替代，而是更粗粒度的“折叠视图”：Gather 往往对应若干轮搜索/读取 loop，Action 往往对应若干轮编辑/执行 loop，Verify 则往往对应若干轮测试/审查 loop。到 10.4 节 再往上一层，就会进入任务级的 P-D-E-R 闭环。
...
10.1.6 节「三代范式演进：从提示词到驾驭工程」介绍了驾驭工程（Harness Engineering）的理论基础——通过闭环控制系统让智能体自主运行并自我纠错。本节聚焦于**具体的组件实现**：在实际的智能体编程产品中，驾驭系统是如何通过消息、工具和指令三层结构落地的。
...
上下文工程的通用原理——持久化、筛选、压缩、隔离——已在第 3.6 节中详细讨论。本节聚焦于 **编程场景下的特殊挑战**：代码库的上下文远比普通对话复杂，文件数量多、依赖关系深、中间产物（编译日志、测试输出）体量大。
...
这里说的“闭环”是**任务级闭环**，不是 10.2 节 的单次 Agent Loop。10.2 的 **Think → Act → Observe** 发生在一次执行迭代内部；本节讨论的是开发者如何把许多轮微观迭代组织成一个可交付、可审查、可回退的工作流。至于审批、沙箱、Hooks 和规则等工程护栏，则放在 10.5 节处理。
...
虽然智能体擅长搜索，但通过统一的“项目说明文件”（推荐命名为 `AGENTS.md`，见附录 12.3）提供项目级的“机器可读说明书”，能减少幻觉与不必要的上下文检索时间。它更适合承载项目目标、代码约定、协作方式、已知陷阱和任务边界，是智能体编程的“地图与说明书”。
...
**技能（Skills）** 是一种将过程性知识封装为可复用知识包的高级设计模式。如果说“工具服务”赋予智能体访问外部系统的能力，那么技能则赋予智能体专业领域知识与稳定工作流。关于技能的完整定义、目录结构、三阶段加载机制和 SkillsBench 实证分析，详见第 4.4 节。
...
驾驭工程（Harness Engineering）的理论基础与六大组成已在第 10.1.6 节详细阐述。本节聚焦**如何在自己的项目中落地驾驭系统**。
