...
系统采用经典的**前后端分离架构**，分为四个层次，如图1 所示：

![图1：智能旅行助手技术架构](images/ch13-01.png){width="60%"}
/// caption
图1：智能旅行助手技术架构
///
...
首先需在首页表单中填写目的地城市、旅行日期、偏好、预算、交通及住宿类型等信息。点击“开始规划”按钮后，系统会显示加载进度条，并很快生成结果页面，如图2 所示。

![图2：旅行助手规划进行页面](images/ch13-02.png){width="60%"}
/// caption
图2：旅行助手规划进行页面
///

随后加载成功，该页面会清晰展示行程概览、预算明细、景点地图、每日行程详情和天气信息，如图3、4 所示。

![图3：旅行助手规划进行页面](images/ch13-03.png){width="60%"}
/// caption
图3：旅行助手规划完成页面
///

![图4：旅行助手规划进行页面](images/ch13-04.png){width="60%"}
/// caption
图4：旅行助手规划完成页面
///
...
![图5：旅行助手规划进行页面](images/ch13-05.png){width="60%"}
/// caption
图5：旅行助手规划完成页面
///
...
![图6：多智能体协作流程](images/ch13-06.png){width="80%"}
///caption
图6：多智能体协作流程
///
...
![图7：amap-mcp-server 工具](images/ch13-07.png){width=80%"}
///caption
图7：amap-mcp-server 工具
///


高德地图 MCP 服务器提供了多种工具，主要分为以下类别，如表1 所示:


| 类别 | 工具 | 功能 |
| ---- | ---- | ---- |
| POI搜索 | amap_maps_text_search | 文本搜索POI |
| POI搜索 | amap_maps_search_detail | 获取POI详情 |
| POI搜索 | amap_maps_around_search | 周边搜索 |
| 路线规划 | amap_maps_direction_walking_by_address | 步行路线 |
| 路线规划 | amap_maps_direction_driving_by_address | 驾车路线 |
| 路线规划 | amap_maps_direction_transit_integrated_by_address | 公交路线 |
| 天气查询 | amap_maps_weather | 天气查询 |
| 地理编码 | amap_maps_geocode | 地址→坐标 |
| 地理编码 | amap_maps_regeocode | 坐标→地址 |
...
![图8：MCP 工具调用流程](images/ch13-08.png){width=80%"}
///caption
图8：MCP 工具调用流程
///
