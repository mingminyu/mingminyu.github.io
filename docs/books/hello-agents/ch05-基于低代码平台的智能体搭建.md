...
当前，智能体与 LLM 应用的低代码平台市场呈现出百花齐放的态势，每个平台都有其独特的定位和优势。选择哪个平台，往往取决于你的核心需求、技术背景以及项目的最终目标。在本章的后续内容中，我们将重点介绍并实操几个各具代表性的平台：Coze、Dify、FastGPT 和 n8n。在此之前，我们先对它们进行一个概要性的介绍。
...
扣子（Coze）是一个超级酷的AI智能体制作工具！也是目前市面上应用最广泛的智能体平台。该平台以其直观的可视化界面和丰富的功能模块，让用户能够轻松创建各种类型的智能体应用，比如能陪你聊天的机器人、自动写故事的创作机，甚至直接帮你将故事变成电影 MV！它的一大亮点在于其强大的生态集成能力。开发完成的智能体可以一键发布到微信、飞书、豆包等主流平台，实现跨平台的无缝部署。对于企业用户而言，Coze 还提供了灵活的 API 接口，支持将智能体能力集成到现有的业务系统中，实现了"搭积木式"的AI应用构建。
...
![图1：扣子智能智能体平台整体示意图](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/coze-01.png){width="80%"}
/// caption
图1：扣子智能智能体平台整体示意图
///
...
首先我们点击左边侧栏的加号就可以看到创建智能体的入口了，这里目前有两类AI应用，一种是创建智能体，另一种叫应用。其中智能体又分为单智能体自主规划模式、单智能体对话流模式和多智能体模式。AI应用也分两种不仅能设计桌面网页端的用户界面，还能轻松搭建小程序和 H5 端的界面，如图2 所示。

![图2：扣子智能体创建入口](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/coze-02.png){width="80%"}
/// caption
图2：扣子智能体创建入口
///

项目空间里是你的智能体仓库，这里放着你所有开发的智能体或复制的智能体/应用，也是在扣子进行智能体开发你最经常来到的地方，如图3 所示。

![图3：扣子智能体项目空间](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/coze-03.png){width="80%"}
/// caption
图3：扣子智能体项目空间
///


资源库是你开发扣子智能体的核心武器库，资源库就会存放你的工作流，知识库，卡片，提示词库等等一系列开发智能体的工具。你能做出什么样的智能体，首先取决于模型的能力，但是最重要的还是要看你怎么给智能体搭配“出装和技能”。模型决定了智能体的下限，但是扣子资源库给了你智能体的能力的无穷上限，让你能够按照自己的想法，开发想象力和脑洞进行智能体的开发，如图4 所示。

![图4：扣子智能体资源库](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/coze-04.png){width="80%"}
/// caption
图4：扣子智能体资源库
///

空间配置包含智能体、插件、工作流和发布渠道的一个统一的管理频道，以及模型管理就是你可以在这里看到你调用的各种大模型，如图5 所示。

![图5：扣子智能体发布渠道](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/coze-03.png){width="80%"}
/// caption
图5：扣子智能体发布渠道
///
...
- **插件集成**: 在 Coze 的插件库中，搜索并添加所需的插件。例如，通过 RSS 插件订阅媒体平台的RSS源（如图6 所示），通过 GitHub 插件追踪开源项目（如图5.7所示），以及通过 arXiv 插件获取最新的学术研究成果（如图5.8所示）。

![图6：媒体平台的RSS源插件](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/coze-06.png){width="80%"}
/// caption
图6：媒体平台的RSS源插件
///


![图7：GitHub插件](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/coze-07.png){width="80%"}
/// caption
图7：GitHub插件
///


![图8：Arxiv插件](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/coze-08.png){width="80%"}
/// caption
图8：Arxiv插件
///
...
- **编排连接**: 在智能体的可视化编排界面中，将这些已配置的信息源插件（例如 `rss_24Hbj`、`searchRepository`、`arxiv` 等）作为数据输入节点，并将其连接至后续的逻辑处理模块（例如大模型模块），以构建完整的数据处理路径，如图9 所示。

![图9：每日AI简报编排流程图](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/coze-09.png){width="80%"}
/// caption
图9：每日AI简报编排流程图
///
...
用户提示（User Prompt）：用户提示用于定义具体的任务指令和数据来源。
...
**多渠道发布**: Coze 提供了将智能体一键发布到多个主流应用平台（如微信、豆包、飞书等）的能力，极大地扩展了智能体的应用场景，如图10 所示。

![图10：扣子平台的多元发布渠道](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/coze-10.png){width="60%"}
/// caption
图10：扣子平台的多元发布渠道
///


智能体发布后，可以在扣子商店中看到我们创建的AI智能体，同时也可以将其集成到AI应用中为用户提供服务，如图11 和图12 所示。在这里也附上[每日AI新闻智能体体验链接](https://www.coze.cn/store/agent/7506052197071962153?bot_id=true&bid=6hkt3je8o2g16)

![图11：AI智能体-每日AI新闻](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/coze-11.png){width="60%"}
/// caption
图11：AI智能体-每日AI新闻
///
...
![图12：AI应用中的每日AI新闻](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/coze-12.png){width="60%"}
/// caption
图12：AI应用中的每日AI新闻
///

发布配置：如果想要发布自己的智能体，还需在发布前，为智能体配置恰当的名称、头像及欢迎语，以提供更友好的用户体验，如图13 和图14 所示。

![图13：为智能体配置基础信息](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/coze-13.png){width="60%"}
/// caption
图13：为智能体配置基础信息
///

![图14：为智能体配置开场白和预设问题](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/coze-14.png){width="60%"}
/// caption
图14：为智能体配置开场白和预设问题
///
...
- **不支持MCP**: 我觉得这是最致命的，尽管扣子的插件市场极其丰富，也极其有吸引力。但是不支持 MCP 可能会成为限制其发展的枷锁，如果放开那将是又一杀手锏。
- **部分插件配置的复杂度高**: 对于需要 API Key 或其他高级参数的插件，用户可能需要具备一定的技术背景才能完成正确的配置。复杂的工作流编排也不仅仅是零基础就可以掌握的，需要一定的 JS 或者 Python 的基础。
- **无法直接导入编排json文件**: 之前扣子是没有导出导入功能的，但是现在付费版是可以导出导入的，但是导出导入的不是像dify,n8n一样的json文件，而是一个zip。也就是说你只能在扣子导出然后扣子导入这个zip。不过你取巧的话也可以选择复制编排，在编排界面 ctrl+a 选中全部 ctrl+c 复制编排，然后到另一个空白的工作流或者其他工作流粘贴编排。
...
Dify 是一个开源的大语言模型（LLM）应用开发平台，融合了后端即服务（BaaS） 和 LLMOps 理念，为从原型设计到生产部署提供全流程支持，如图15 所示。它采用分层模块化架构，分为数据层、开发层、编排层和基础层，各层解耦便于扩展。
...
![图15：Dify官网](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/dify-01.png){width="60%"}
/// caption
图15：Dify官网
///


**Marketplace 插件生态**：​Dify Marketplace 提供了一站式插件管理和一键部署功能，使开发者能够发现、扩展或提交插件，为社区带来更多可能​，如图16 所示。

![图16：Dify Marketplace插件生态](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/dify-02.png){width="60%"}
/// caption
图16：Dify Marketplace插件生态
///
...
![图17：Dify 智能体搭建主页](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/dify-14.png){width="60%"}
/// caption
图17：Dify 智能体搭建主页
///


![图18：Dify 智能体搭建主页](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/dify-18.png){width="60%"}
/// caption
图18：Dify 官方模板库
///


![图19：Dify 知识库](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/dify-15.png){width="60%"}
/// caption
图19：Dify 知识库
///

![图20：Dify 插件市场](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/dify-16.png){width="60%"}
/// caption
图20：Dify 插件市场
///

![图21：Dify 大模型配置](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/dify-17.png){width="60%"}
/// caption
图21：Dify 大模型配置
///
...
在构建智能体之前，需要先完成必要的插件安装和 MCP 配置。如图22 所示，这些是本案例所需的核心插件。

![图22：Dify 插件安装配置](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/dify-19.png)
/// caption
图22：Dify 插件安装配置
///
...
接下来配置 MCP（Model Context Protocol）。关于 MCP 的详细原理这里不展开，我们重点演示如何使用云端部署的 MCP 服务。本案例使用国内的魔搭社区 MCP 市场进行演示，如图23 所示。

![图23：魔搭社区MCP市场](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/dify-20.png){width="80%"}
/// caption
图23：魔搭社区MCP市场
///
...
![图24：高德MCP配置示例](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/dify-21.png){width="80%"}
/// caption
图24：高德MCP配置示例
///
...
![图25：智能体编排](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/dify-12.png){width="80%"}
/// caption
图25：智能体编排
///
...
**日常助手模块**：这是一个基础的对话模块，配置大语言模型和时间工具，作为兜底的通用问答服务。

``` markdown linenums="1" title="提示词配置"
# Role: 日常问题咨询专家
...
![图26：日常助手](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/dify-03.png){width="80%"}
/// caption
图26：日常助手
///


**文案优化模块**：根据 OpenAI 的数据报告，超过60%的用户使用 ChatGPT 进行文本优化相关任务，包括润色、修改、扩写、缩写等。因此，文案优化是高频需求场景，我们将其作为第二个核心功能模块。
...
![图27：日常助手](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/dify-04.png){width="80%"}
/// caption
图27：日常助手
///

**多模态生成模块**：图片和视频生成是另一个高频应用场景。随着豆包生图、Google Imagen 等模型的进化，以及可灵、Google Veo 3、seedance2.0 等视频生成技术的突破，多模态内容生成的质量已达到实用水平。
...
![图28：生图设置](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/dify-13.png){width="80%"}
/// caption
图28：生图设置
///

![图29：生图助手](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/dify-05.png){width="80%"}
/// caption
图29：生图助手
///
...
![图30：视频助手](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/dify-06.png){width="80%"}
/// caption
图30：[视频助手](https://pub-f5ed2046361c4244878e5984bdb564de.r2.dev/9af7c33d-5c82-4b14-8fb3-a4e426e8ee5a.mp4)
///


**数据查询与分析模块**：数据处理是智能体的重要能力之一。本模块演示如何在 Dify 中连接数据库，实现数据查询和可视化分析。
...
配置数据库连接信息（IP地址、数据库名称、端口、账号、密码等），如图31 所示。查询结果需要通过大模型节点进行整理，转换为易于理解的自然语言输出。


![图31：数据库配置](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/dify-22.png){width="80%"}
/// caption
图31：数据库配置
///


``` markdown linenums="1" title="提示词设置"
# 一、 角色人设（Role）
...
![图32：数据查询助手](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/dify-07.png){width="80%"}
/// caption
图32：数据查询助手
///
...
![图33：数据分析助手](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/dify-08.png){width="80%"}
/// caption
图33：数据分析助手
///
...
**MCP 工具集成**：最后是 MCP 工具的集成应用。在前面我们已经完成了 MCP 的配置，现在将其集成到智能体中。配置步骤如下：
...
![图34：智能体的mcp配置](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/dify-23.png){width="80%"}
/// caption
图34：智能体的mcp配置
///


高德助手、饮食助手和新闻助手的效果分别如图35、图36 和图37 所示。

![图35：高德助手](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/dify-09.png){width="80%"}
/// caption
图35：高德助手
///

![图36：饮食助手](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/dify-10.png){width="80%"}
/// caption
图36：饮食助手
///


![图37：饮食助手](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/dify-11.png){width="80%"}
/// caption
图37：新闻助手
///
...
进入 FastGPT 官网，首先映入眼帘的是其简洁有力的产品宣言——"企业级 AI 生产力引擎"，强调构建安全、可控的企业级 AI Agent，如图38 所示。

![图38：FastGPT 官网首页](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/fastgpt-01.png){width="80%"}
/// caption
图38：FastGPT 官网首页
///


登录平台后，可以看到其清晰的工作台布局。左侧导航栏将核心功能划分为 对话门户、工作台、知识库和账号四大模块。其中 Agent 模块又细分为工作流、对话 Agent 和对话 Agent V2(Beta) 三种类型，方便用户根据业务场景选择合适的构建模式。主区域则提供了"从模板新建"的快捷入口，内置了销售陪练大师、文档翻译助手、行业趋势洞察简报等官方模板；下方是用户自己创建的 Agent 列表，如图39 所示。

![图39：FastGPT 平台主界面](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/fastgpt-02.png){width="80%"}
/// caption
图39：FastGPT 平台主界面
///
...
![图40：FastGPT 免费版套餐与用量](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/fastgpt-03.png){width="80%"}
/// caption
图40：FastGPT 免费版套餐与用量
///
...
![图41：FastGPT 知识库文件管理](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/fastgpt-12.png){width="80%"}
/// caption
图41：FastGPT 知识库文件管理
///
...
![图42：FastGPT 知识库数据处理参数设置](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/fastgpt-14.png){width="80%"}
/// caption
图42：FastGPT 知识库数据处理参数设置
///


上传完成后，用户可以查看文件被分块后的具体内容。如图43 所示，以"英语四年级下册全册电子书.pdf"为例，平台展示了每个分块的文本预览，同时右侧元数据面板显示了文件大小（62MB）、原文长度（37797 字符）、处理模式（分块存储）、图片索引状态等关键信息。这种透明化的分块展示，方便开发者进行知识库的调试与优化。

![图43：知识库文件分块详情与元数据](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/fastgpt-13.png){width="80%"}
/// caption
图43：知识库文件分块详情与元数据
///

除了知识库，FastGPT 在工具集成方面也紧跟生态趋势。平台原生支持 MCP（Model Context Protocol）工具，用户可以在"我的工具"模块中统一管理各类 MCP 服务。如图44 所示，在"ai 金融"文件夹下，我们已经配置了中文趋势聚合、实时股票 MCP、且慢基金 MCP、Minimax-MCP、BI 画图工具等多个 MCP 工具，这些工具将赋予智能体调用外部实时数据和专业服务的能力。

![图44：FastGPT MCP 工具管理](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/fastgpt-13.png){width="80%"}
/// caption
图44：FastGPT MCP 工具管理
///
...
如图45 所示，我们可以在魔搭社区（ModelScope）的 MCP 市场中找到"可视化图表 MCP Server"。该服务基于 TypeScript 开发，兼容 MCP 协议，提供了生成面积图、柱状图、饼图等多种图表的能力，能够将枯燥的数据转化为直观的可视化结果。

![图45：魔搭社区可视化图表 MCP Server](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/fastgpt-05.png){width="80%"}
/// caption
图45：魔搭社区可视化图表 MCP Server
///

另外，如图46 所示，阿里云百炼平台也提供了丰富的官方 MCP 服务。在 MCP 管理页面中，我们可以找到"今日投资-金融实..."和"且慢"等金融类 MCP 服务，以及股票实时行情查询、万相-视频生成等工具。将这些服务添加到 FastGPT 的 MCP 工具库后，智能体便能在对话中按需调用它们。

![图46：阿里云百炼 MCP 管理](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/fastgpt-05.png){width="80%"}
/// caption
图46：阿里云百炼 MCP 管理
///
...
如图47 所示，"智能投顾助手"的完整工作流包含了多个处理分支：用户意图识别、知识库检索、风险问卷收集、MCP 工具调用、报告生成等。整个工作流呈现出清晰的模块化结构，数据在不同节点间有序流转。这种可视化的编排方式，让开发者能够直观地理解和调试智能体的决策路径。

![图47：智能投顾助手工作流编排](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/fastgpt-05.png){width="80%"}
/// caption
图47：智能投顾助手工作流编排
///
...
![图48：智能投顾助手对话界面](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/fastgpt-08.png){width="80%"}
/// caption
图48：智能投顾助手对话界面
///
...
![图49：投资策略分析报告](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/fastgpt-08.png){width="80%"}
/// caption
图49：投资策略分析报告
///


对于实时数据查询场景，如图50 所示，当用户询问"查询现在贵州茅台的股价信息"时，智能体会自动调用 MCP 工具（`get_stock_quote_realtime`）获取实时行情数据。返回结果包含标题、数据来源、关键要点（开盘价、最高价、日内价格区间、成交量、总市值、流通市值等），以及潜在影响分析和建议行动。这种结构化、专业化的输出，体现了 Agent 工具调用能力的实际价值。

![图50：投资策略分析报告](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/fastgpt-10.png){width="80%"}
/// caption
图50：投资策略分析报告
///
...
![图51：市盈率与市净率概念解析](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/fastgpt-11.png){width="80%"}
/// caption
图51：市盈率与市净率概念解析
///

### 4.3 FastGPT 的优势与局限性分析
...
关于 n8n 的环境配置和最基础的使用，在项目的 `Additional-Chapter` 文件夹下制作了文档，这里就不过多介绍。在上一节中，我们了解了 n8n 的基本概念。这个案例将清晰地展示现代 AI Agent 与传统自动化工作流的核心区别。传统流程是线性的，而我们即将构建的 Agent 将能够接收用户邮件，通过一个核心的 AI Agent 节点 进行“思考”，自主理解用户意图，并在多个可用“工具”中进行决策和选择，最终自动生成并发送高度相关的回复。

整个过程模拟了一个更高级的决策逻辑：`接收 -> AI Agent (思考 -> 决策 -> 工具调用) -> 回复`，如图52所示。

![图52：一体化智能邮件 Agent 架构示意图](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/n8n-01.png){width="80%"}
/// caption
图52：一体化智能邮件 Agent 架构示意图
///
...
![图53：在 Code 节点中定义知识源](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/n8n-02.png){width="80%"}
/// caption
图53：在 Code 节点中定义知识源
///
...
![图54：在 Code 中数据进行向量化](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/n8n-03.png){width="80%"}
/// caption
图54：在 Code 中数据进行向量化
///
...
![图55：在 Code 中数据存入向量存储](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/n8n-04.png){width="80%"}
/// caption
图55：在 Code 中数据存入向量存储
///

完成配置后，手动执行一次这个流程。成功后，您的私有知识就加载到 n8n 的内存中了，如图56 所示。


![图56：完整的知识库加载工作流](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/n8n-05.png){width="80%"}
/// caption
图56：完整的知识库加载工作流
///
...
新建一个工作流，命名为 `Agent: Customer Support`。使用 `Gmail` 节点作为触发器，将其 Event 设置为 `Message Received`，并配置好你的邮箱账号。这样，每当有新邮件进入收件箱时，该工作流就会被自动触发，如图57 所示。

![图57：新建Gmail节点图](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/n8n-06.png){width="80%"}
/// caption
图57：新建Gmail节点图
///
...
![图58：Gmail账号加载成功图](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/n8n-07.png){width="80%"}
/// caption
图58：Gmail账号加载成功图
///

现在我们可以点击`Fetch Test Event`获取邮件了，如图59 所示！

![图59：获取实时邮件图](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/n8n-08.png){width="80%"}
/// caption
图59：获取实时邮件图
///
...
![图60：AI Agent节点设置图](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/n8n-09.png){width="80%"}
/// caption
图60：AI Agent节点设置图
///
...
其中，对于AI Agent节点，我们需要填的主要是 `User Message` 和 `System Message` 部分，如图61 所示。

![图61：AI Agent节点详解图](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/n8n-10.png){width="80%"}
/// caption
图61：AI Agent节点详解图
///
...
![图62：Simple Vector Store工具配置](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/n8n-11.png){width="80%"}
/// caption
图62：Simple Vector Store工具配置
///
...
对于Memory唯一需要注意的是，这里我们使用每个邮箱的线程名作为唯一标识，能保证存储的唯一性，设置的 Key 为 `{{ $('Gmail').item.json.threadId }}`
...
![图63：最终回复工具图示](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/n8n-12.png){width="80%"}
/// caption
图63：最终回复工具图示
///

并且发送成功的同时，也能在个人邮箱收到真实的返回邮件信息，如图64 所示。


![图64：个人邮箱返回邮件格式](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/n8n-13.png){width="80%"}
/// caption
图64：个人邮箱返回邮件格式
///
...
![表2：n8n 平台的优势与局限性总结](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/5-figures/n8n-14.png){width="80%"}
/// caption
表2：n8n 平台的优势与局限性总结
///
