...
在准备好 LLM 客户端后，我们将构建第一个，也是最经典的一个智能体范式 **ReAct**(Reason + Act)。ReAct 由 Shunyu Yao 于 2022 年提出[^1]，其核心思想是模仿人类解决问题的方式，将**推理** (Reasoning) 与**行动**(Acting) 显式地结合起来，形成一个“思考-行动-观察”的循环。
...
![图1：ReAct 范式中的“思考-行动-观察”协同循环](images/ch04-01.png){width="80%"}
/// caption
图1：ReAct范式中的“思考-行动-观察”协同循环
///
...
因此我们将构建一个具备**使用外部工具**能力的 ReAct 智能体，来回答一个大语言模型仅凭自身知识库无法直接回答的问题。例如：“华为最新的手机是哪一款？它的主要卖点是什么？” 这个问题需要智能体理解自己需要上网搜索，调用工具搜索结果并总结答案。
...
在上述代码中，首先会检查是否存在 `answer_box`（Google 的答案摘要框）或 `knowledge_graph`（知识图谱）等信息，如果存在，就直接返回这些最精确的答案。如果不存在，它才会退而求其次，返回前三个常规搜索结果的摘要。这种“智能解析”能为LLM提供质量更高的信息输入。
...
![图2：Plan-and-Solve 范式的两阶段工作流](images/ch04-02.png){width="80%"}
/// caption
图2：Plan-and-Solve 范式的两阶段工作流
///
...
请严格按照以下格式输出你的计划,``` python linenums="1"与```作为前后缀是必要的:
``` python linenums="1"
["步骤1", "步骤2", "步骤3", ...]
```
"""
...
class Planner:
    ...
    def plan(self, question: str) -> list[str]:
        ...
        # 解析LLM输出的列表字符串
        try:
            # 找到``` python linenums="1"和```之间的内容
            plan_str = response_text.split("``` python linenums="1"")[1].split("```")[0].strip()
...
![图3：Reflection 机制中的“执行-反思-优化”迭代循环](images/ch04-03.png){width="80%"}
/// caption
图3：Reflection 机制中的“执行-反思-优化”迭代循环
///
