...
> 原文地址: https://nextjs.org/docs/app/getting-started/project-structure
...
要将特定路由组织到一个布局中，请创建一个新的路线组（例如 `(shop)`）并将共享相同布局的路线移动到该组中（例如`account`和`cart`）。组外的路线不会共享布局（例如 `checkout`）。

![](https://mingminyu.github.io/webassets/images/nextjs/14.png)

### 3.5 在特定路由上加载骨架

要通过 `loading.js` 文件将加载框架应用到特定路由，请创建一个新的路由组（例如 `/(overview)`），然后将您的`loading.tsx` 移动到该路由组内。

![](https://mingminyu.github.io/webassets/images/nextjs/15.png)

现在， `loading.tsx` 文件将仅应用于您的 **仪表板 → 概述** 页面，而不是所有仪表板页面，而不影响 URL 路径结构。

### 3.6 创建多个根布局

要创建多个根布局，请删除顶级 `layout.js` 文件，并在每个路由组内添加一个 `layout.js` 文件。这对于将应用程序划分为具有完全不同的 UI 或体验的部分非常有用。需要将 `<html>`和`<body>`标记添加到每个根布局中。

![](https://mingminyu.github.io/webassets/images/nextjs/16.png)
